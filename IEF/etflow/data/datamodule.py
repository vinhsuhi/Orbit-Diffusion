...
class BaseDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if len(self.train_dataset) < self.dataloader_args["batch_size"]:
            repeat_factor = -(-self.dataloader_args["batch_size"] // len(self.train_dataset)) 
            self.train_dataset = ConcatDataset([self.train_dataset] * repeat_factor)

        if len(self.val_dataset) < self.dataloader_args["batch_size"]:
            repeat_factor = -(-self.dataloader_args["batch_size"] // len(self.val_dataset)) 
            self.val_dataset = ConcatDataset([self.val_dataset] * repeat_factor)
