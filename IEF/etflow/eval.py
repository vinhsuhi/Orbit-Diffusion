...
def main(
    ...
    model = instantiate_model(config["model"], config["model_args"], config["train_mode"], config["predict_mode"], config["num_rotations"])
    ...
    for i, idx in tqdm(enumerate(indices), total=len(indices)):
        ...
        if os.path.isfile(path):
            log.info(f"Already generated positions to file for smiles {smiles} at {path}")
            continue
    ...
    l = set([int(x.split(".pkl")[0]) for x in os.listdir(output_dir) if x.split(".pkl")[0].isnumeric()])
    ...
    # save_pkl(os.path.join(output_dir, "ti÷mes.pkl"), times)
    ...
    for index in tqdm(l):
        ...
        data_list[smiles] = Data(
            smiles=smiles, pos_ref=pos_ref, rdmol=item.mol, pos_gen=pos_gen, remove_hs=config["remove_hs"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--predict_mode", type=str, default="v")
