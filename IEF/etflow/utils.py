...
def instantiate_model(
    model_type: str, model_args: dict, train_mode: str, predict_mode: str, num_rotations: int = 200
) -> LightningModule:
    model_args["train_mode"] = train_mode
    model_args["num_rotations"] = num_rotations
...
def instantiate_dataset(dataset_type: str, dataset_args: dict, **kwargs) -> LightningDataModule:
...
def instantiate_datamodule(
    datamodule_type: str, datamodule_args: dict, **kwargs,
) -> BaseDataModule:    
    dataset = instantiate_dataset(
        datamodule_args["dataset"], datamodule_args["dataset_args"], **kwargs,
    )
...
def instantiate_trainer(
    ...
    if trainer_type == "Trainer":
        trainer = Trainer(**trainer_args, enable_progress_bar=True, logger=logger, callbacks=callbacks)
    else:
        raise NotImplementedError
...
def instantiate_callbacks(callbacks: list) -> List[Callback]:
    ...
    for callback_dict in callbacks:
        ...
        elif callback_dict["callback"] == "TQDMProgressBar":
            final_callbacks.append(TQDMProgressBar(**callback_dict["callback_args"]))
        else:
            raise NotImplementedError
