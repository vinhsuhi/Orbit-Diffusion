...
def get_best_rmsd(probe, ref, use_alignmol=False, use_reflect=False):
    def mirror_molecule(mol):
        mirrored = Mol(mol)
        conf = mirrored.GetConformer()
        for i in range(conf.GetNumAtoms()):
            pos = conf.GetAtomPosition(i)
            conf.SetAtomPosition(i, (-pos.x, pos.y, pos.z))  # Reflect across YZ plane
        return mirrored
    ...
    try:
        if use_alignmol:
            rmsd = MA.AlignMol(probe, ref)
            if use_reflect:
                mirrored_probe = mirror_molecule(probe)
                rmsd_mirrored = MA.AlignMol(mirrored_probe, ref)
                rmsd = min(rmsd, rmsd_mirrored)
        else:
            rmsd = MA.GetBestRMS(probe, ref)
            if use_reflect:
                mirrored_probe = mirror_molecule(probe)
                rmsd_mirrored = MA.GetBestRMS(mirrored_probe, ref)
                rmsd = min(rmsd, rmsd_mirrored)
    except:  # noqa
        rmsd = np.nan
...
def get_rmsd(ref_mol: Mol, gen_mols: List[Mol], useFF=False, use_alignmol=False, use_reflect=False):
    ...
    for i in range(num_gen):
        ...
        rmsd_vals.append(get_best_rmsd(gen_mol, ref_mol, use_alignmol=use_alignmol, use_reflect=use_reflect))
...
def worker_fn(job, useFF=False, use_alignmol=False, use_reflect=False):
    smi, i_true, ref_mol, gen_mols = job
    rmsd_vals = get_rmsd(ref_mol, gen_mols, useFF=useFF, use_alignmol=use_alignmol, use_reflect=use_reflect)
...
class CovMatEvaluator(object):
    ...
    def __call__(self, packed_data_list, start_idx=0):
        ...
        for data in packed_data_list:
            remove_hs = data.get("remove_hs", False)
            ...
            smiles = data["smiles"]
            mol = dm.to_mol(smiles, remove_hs=remove_hs, ordered=True)
        ...
        fn = partial(
            worker_fn, useFF=self.use_force_field, use_alignmol=self.use_alignmol, use_reflect=self.use_reflect
        )
