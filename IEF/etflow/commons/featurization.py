...
class MoleculeFeaturizer:
    ...
    def get_mol(self, smiles: str, remove_hs: bool = False) -> Mol:
        mol = dm.to_mol(smiles, remove_hs=remove_hs, ordered=True)
        return mol

    @cache_decorator
    def get_atom_features(self, smiles: str, use_ogb_feat: bool = True, remove_hs: bool = False) -> torch.Tensor:
        # compute atom features
        mol = self.get_mol(smiles, remove_hs=remove_hs)
        atom_features = self.get_atom_features_from_mol(mol, use_ogb_feat=use_ogb_feat)
        return atom_features

    @cache_decorator
    def get_atomic_numbers(self, smiles: str, remove_hs: bool = False) -> torch.Tensor:
    ...
    @cache_decorator
    def get_chiral_centers(self, smiles: str, remove_hs: bool = False) -> torch.Tensor:
    ...
    @cache_decorator
    def get_mol_with_conformer(self, smiles: str, positions: torch.Tensor, remove_hs : bool = False) -> Mol:
        mol = self.get_mol(smiles, remove_hs=remove_hs)
    ...
    @cache_decorator
    def get_edge_index(
        self, smiles: str, use_edge_feat: bool, no_reverse: bool = False, remove_hs : bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Returns edge index and edge attributes for a given smiles."""
        # compute edge index
        mol = self.get_mol(smiles, remove_hs=remove_hs)
        edge_index, edge_attr = self.get_edge_index_from_mol(
            mol, use_edge_feat=use_edge_feat, no_reverse=no_reverse
        )
    ...
    def get_edge_index_from_mol(
        self, mol: Mol, use_edge_feat: bool = False, no_reverse: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Returns edge index and edge attributes for a given mol object."""
        edge_index, edge_attr = compute_edge_index(mol, with_edge_attr=use_edge_feat, no_reverse=no_reverse)
        return edge_index, edge_attr

    def get_data_from_smiles(self, smiles: str) -> Data:
        # TODO: check this one for prediction
