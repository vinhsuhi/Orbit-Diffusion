...
def prepare_text(structure, dataset, all_attributes, properties):
    ...
    for attr in all_attributes:
        ...
        elif attr == "pretty_formula":
            prompt += f"{prompt_lookup[attr]} {str(structure.composition.reduced_formula)}. "
        elif attr == "system":
            prompt += f"{prompt_lookup[attr]} {str(sga.get_crystal_system())}. "
        elif attr == "spacegroup":
            prompt += f"{prompt_lookup[attr]} {str(sga.get_space_group_number())}. "
        elif attr in ["formation_energy_per_atom", "band_gap", "e_above_hull", "heat_ref", "energy_per_atom"]:
...
def preprocess(input_file, niggli, primitive, graph_method, prop_list, all_attributes, dataset, use_space_group = False, tol=0.01):
