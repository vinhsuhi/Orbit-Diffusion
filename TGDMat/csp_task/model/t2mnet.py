...
from model.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPLayer(nn.Module):
    """ Message passing layer for t2mnet."""
...
class T2MNet(nn.Module):
    ...
    def __init__(
        ...
        self.atom_latent_emb = nn.Linear(hidden_dim + 256 + 256, hidden_dim)
        ...
        for i in range(0, num_layers):
            self.add_module("csp_layer_%d" % i, CSPLayer(hidden_dim, self.act_fn, self.dis_emb, ln=ln, ip=ip))
    ...
    def forward(self, text_emb, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        text_emb_per_atom = text_emb.repeat_interleave(num_atoms, dim=0)
        node_features = torch.cat([node_features, text_emb_per_atom, t_per_atom], dim=1)
