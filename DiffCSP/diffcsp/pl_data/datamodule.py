...
class CrystDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage is None or stage == "fit":
            ...
            if len(self.train_dataset) < self.batch_size.train:
                repeat_factor = -(-self.batch_size.train // len(self.train_dataset)) 
                self.train_dataset = ConcatDataset([self.train_dataset] * repeat_factor)
            
            if self.beta_scheduler is not None:
                self.train_dataset.set_schedulers(self.beta_scheduler, self.sigma_scheduler)
            ...
            for i in range(len(self.val_datasets)):
                if len(self.val_datasets[i]) < self.batch_size.val:
                    repeat_factor = -(-self.batch_size.val // len(self.val_datasets[i])) 
                    self.val_datasets[i] = ConcatDataset([self.val_datasets[i]] * repeat_factor)
                
                if self.beta_scheduler is not None:
                    self.val_datasets[i].set_schedulers(self.beta_scheduler, self.sigma_scheduler)
