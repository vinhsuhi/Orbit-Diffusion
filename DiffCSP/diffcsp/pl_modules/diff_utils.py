...
def sigma_norm_stable(sigma, T=1.0, sn = 10000):
    sigmas = sigma[None, :].repeat(sn, 1)
    x_sample = sigma * torch.randn_like(sigmas)
    x_sample = x_sample % T
    normal_ = d_log_p_wrapped_normal_stable(x_sample, sigmas, T = T)
...
class BetaScheduler(nn.Module):
    ...
    def uniform_sample_t(self, batch_size, device, num_bins=1):
        if num_bins == 1:
            ts = np.random.choice(np.arange(1, self.timesteps + 1), batch_size)
        else:
            bins = np.linspace(1, self.timesteps + 1, num_bins + 1, dtype=int)
            ts = []
            samples_per_bin = batch_size // num_bins
            remainder = batch_size % num_bins
            
            for i in range(num_bins):
                start, end = bins[i], bins[i + 1]
                count = samples_per_bin + (1 if i < remainder else 0)  # distribute remainder
                if end > start:
                    ts.append(np.random.choice(np.arange(start, end), size=count))
                else:
                    ts.append(np.full(count, start))  # degenerate bin, pick start
            
            ts = np.concatenate(ts)
            np.random.shuffle(ts)  # optional: shuffle after bin sampling to avoid ordering
