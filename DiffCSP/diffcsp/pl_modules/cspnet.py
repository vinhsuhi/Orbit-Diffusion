...
class CSPNet(nn.Module):
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        edges, frac_diff = self.gen_edges(num_atoms, frac_coords, lattices, node2graph) # type: ignore
        ...
        for i in range(0, self.num_layers):
            node_features = self._modules["csp_layer_%d" % i](node_features, frac_coords, lattices, edges, edge2graph, frac_diff = frac_diff) # type: ignore
        ...
        graph_features = scatter(node_features, node2graph, dim = 0, reduce = self.agg_type)
