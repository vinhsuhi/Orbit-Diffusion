...
def get_symmetry_info(crystal, tol=0.01):
    ...
    print(anchors,coords,  matrices, len(c.atom_sites), [len(site.wp) for site in c.atom_sites])
...
def generate_permuted_coords(coords, atom_types, max_perms=5000):
    unique_types, inverse_indices = np.unique(atom_types, return_inverse=True)
    indices_by_type = {t: np.where(inverse_indices == i)[0] for i, t in enumerate(unique_types)}    
    ...
    group_permutations = {t: itertools.permutations(indices_by_type[t])
                          for t in unique_types if len(indices_by_type[t]) > 1}
    all_permutations = itertools.product(*group_permutations.values())
    all_permutations = itertools.islice(all_permutations, max_perms)
    permuted_coords = []

    for perm in all_permutations:
        perm_map = np.arange(len(coords)) 
        for perm_group in perm:
            for orig, new in zip(indices_by_type[atom_types[perm_group[0]]], perm_group):
                perm_map[orig] = new 
        permuted_coords.append(coords[perm_map])

    return np.array(permuted_coords)
...
def build_crystal_graph(crystal, graph_method='crystalnn'):
    ...
    permuted_frac_coords = generate_permuted_coords(frac_coords, atom_types)

    return frac_coords, atom_types, lengths, angles, edge_indices, to_jimages, num_atoms, permuted_frac_coords
