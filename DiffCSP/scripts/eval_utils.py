...
def load_model(model_path, load_data=False, testing=True, load_best=False, load_at=None):
    with initialize_config_dir(str(model_path)):
        cfg = compose(config_name='hparams')
        ...
        if len(ckpts) > 0:
            if not load_best:
                if load_at is not None:
                    # Try loading the specified epoch
                    ckpt = next((str(ck) for ck in ckpts if f'epochepoch={load_at:03d}' in ck.parts[-1]), None)
                if ckpt is None:
                    # If no load_at or specified epoch not found, try loading 'last'
                    for ck in ckpts:
                        if 'last' in ck.parts[-1]:
                            ckpt = str(ck)
                            break

            # If load_best or no valid checkpoint found yet, load the best one
            if ckpt is None:
                selected_ckpts = [ckpt for ckpt in ckpts if 'last' not in ckpt.parts[-1] and not ckpt.parts[-1].startswith("epochepoch")]
                ckpt_epochs = np.array([int(ckpt.parts[-1].split('-')[0].split('=')[1]) for ckpt in selected_ckpts])
                ckpt = str(selected_ckpts[ckpt_epochs.argsort()[-1]])
